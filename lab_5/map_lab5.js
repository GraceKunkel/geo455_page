...
// line
var line = L.polyline(coords, {color: "green", weight: 7}).bindPopup("Travel Path");
line.addTo(mymap);
...
//// event listener for click events to show lat long on the map
...
// International Space Station (ISS) marker with a custom icon
  var issIcon = L.icon({
...
// Lat/long based on the updated reading everytime
  async function getISS() {
    var response = await fetch(api_url);
    var data = await response.json();
    var { latitude, longitude } = data;

// location based on the updated reading but keep the map view in default center
